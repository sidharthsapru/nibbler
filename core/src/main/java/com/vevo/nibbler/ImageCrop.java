...
public class ImageCrop {
    ...
    private final static double Default_Tolerance = 0.1;
    ...
    public static BufferedImage cropBlackBorder(BufferedImage source, double tolerance) {
        ...
        /*
            image can have no  border or a border  too small to detect.
            Image cropped is almost same size don't resize.
         */
        if(almostEqual((bottomX-topX+1), width, 5)
                && almostEqual((bottomY - topY + 1), height, 5)) {
            return source;
        }
    ...
    private static boolean almostEqual(int expected, int actual, int tolerance) {
